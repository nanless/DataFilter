...
def walk_csv_rows(segment_file_path, queue, batch_size=1):
    batch = []
...
def process_rows(queue, device, original_folder, model_name, df, target_folder, batch_size=1):
    ...
    while True:
        ...
        for row in rows:
            if os.path.exists(os.path.join(original_folder, row[0] + ".flac")):
                real_classs = []
                classes = row[3:]
                for c in classes:
        ...
        for row, response in zip(valid_rows, responses):
            ...
            if response == "no":
                ...
                shutil.copy(os.path.join(original_folder, row[0] + ".flac"), os.path.join(target_folder, response))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--original_folder", type=str, default="/data1/data/noise/audioset/data/downloads/audio/bal_train")
    parser.add_argument("--model_name", type=str, default="Qwen/Qwen2-7B-Instruct")
    parser.add_argument("--segment_file_path", type=str, default="/data1/data/noise/audioset/balanced_train_segments.csv")
    parser.add_argument("--class_pair_file_path", type=str, default="/data1/data/noise/audioset/class_labels_indices.csv")
    parser.add_argument("--target_folder", type=str, default="/data1/data/noise/audioset/selected/bal_train")
    ...
    segment_file_path = args.segment_file_path
    class_pair_file_path = args.class_pair_file_path
    ...
    walker_process = Process(target=walk_csv_rows, args=(segment_file_path, queue, batch_size))
    ...
    if torch.cuda.is_available():
        ...
        if "NVIDIA" in device_name.upper():
            ...
            for i in range(num_processes):
                p = Process(target=process_rows, args=(queue, devices[i % len(devices)], original_folder, model_name, df, target_folder, batch_size))
                p.start()
            ...
            process_rows(queue, devices[0], original_folder, model_name, df, target_folder, batch_size)
            walker_process.join()
        else:
            print("未定义的GPU类型")
    else:
